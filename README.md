...
Χατζηβασιλείου Αριστοτέλης (@tlkcexe)

Βασιλειάδης Σωτήριος (@Digbasanis)

Κοζάρης Αλέξανδρος (@alkozaris)

Οδηγίες & Συνοδευτική Τεκμηρίωση (Project Documentation)

Στον κεντρικό κατάλογο του αποθετηρίου (repository) έχουν προστεθεί όλα τα απαραίτητα αρχεία τεκμηρίωσης και οδηγοί για την τελική παρουσίαση:

Οδηγός Χρήστη & Δημιουργού: Ολοκληρωμένος Οδηγός Χρήστη (διαθέσιμες εντολές) και Οδηγός Δημιουργού (περιγραφή δομής JSON για τη σύνταξη νέων σεναρίων).

Αναλυτικό Σκονάκι Παρουσίασης: Αναλυτικό «Σκονάκι» Παρουσίασης με πλήρη επεξήγηση της αρχιτεκτονικής, των σχεδιαστικών προτύπων (Command Pattern, κ.λπ.) και της λειτουργίας κάθε αρχείου κώδικα (ΤΙ κάνει και ΠΩΣ).

Λύση για το The Gallery Mystery: Αναλυτικός βήμα-βήμα οδηγός (λύση) για την πλήρη ολοκλήρωση του παιχνιδιού The Gallery Mystery (demo_game.json).

Λύση για το Cold Horizon Station: Αναλυτικός βήμα-βήμα οδηγός (λύση) για την πλήρη ολοκλήρωση του παιχνιδιού Cold Horizon Station (demo_game2.json).

Οδηγίες Εκτέλεσης της Εφαρμογής

Η μηχανή PorkEngine βασίζεται στη Java και χρησιμοποιεί την εξωτερική βιβλιοθήκη Gson για το parsing των αρχείων JSON.

Επιλογή 1: Εκτέλεση μέσω IDE (VS Code, IntelliJ, Eclipse)

Ανοίξτε τον φάκελο PorkEngine στο περιβάλλον ανάπτυξης (IDE) που χρησιμοποιείτε.

Εάν η βιβλιοθήκη Gson δεν αναγνωρίζεται αυτόματα, κάντε δεξί κλικ στο αρχείο lib/gson-2.10.1.jar και επιλέξτε Add as Library ή Add to Build Path.

Ανοίξτε το αρχείο src/Main.java, κάντε δεξί κλικ και επιλέξτε Run.

Επιλογή 2: Εκτέλεση μέσω Τερματικού (Κονσόλας)

Ανοίξτε το τερματικό σας, μεταβείτε στον φάκελο PorkEngine και εκτελέστε την αντίστοιχη εντολή:

Για Windows (CMD / PowerShell):
...
ΛΑ-4 (Σύστημα Εντολών): Επεκτάσιμο, data-oriented Command System που υποστηρίζει συνώνυμα ρημάτων (π.χ. take/grab/pick up) και πολύπλοκες προθετικές φράσεις με πολλαπλά αντικείμενα (π.χ. "give toolkit to engineer").
...
Model Layer: Αφηρημένες αναπαραστάσεις των οντοτήτων. Περιέχει τα interfaces και τις abstract κλάσεις (Room, Exit, Item, Player).

Engine Core: Περιλαμβάνει την κλάση GameEngine (κεντρικό game loop), το GameState (συντονιστής προόδου) και το ConsoleUI για την απομόνωση του Input/Output.

Game Loader System: Υπεύθυνο για την εξωτερική φόρτωση δεδομένων. Περιλαμβάνει τον JsonGameLoader και τις δομές in-memory αποθήκευσης (RoomRegistry, ItemRegistry).

Command System: Το σύστημα επεξεργασίας κειμένου. Απαρτίζεται από τον CommandParser (tokenizer & synonym resolver), το data-driven CommandRegistry (verb-to-command mapping), τον CommandDispatcher και τις διακριτές υλοποιήσεις εντολών (π.χ. GoCommand, TakeCommand, MultiObjectCommand).

Event & Condition System: Διαχειρίζεται τους περιορισμούς του περιβάλλοντος (π.χ. locked states) και τα GameFlags για τον τερματισμό του παιχνιδιού.

NPC System (Bonus): Υποσύστημα που ενσωματώνει το NpcStateMachine (states/transitions), το NpcRegistry και τον NpcStagingManager.
...
ΠΧ-04 (Σύνθετη Εντολή - Prepositions & Smart Use): Ο παίκτης εισάγει use badge. Το UseCommand εκτελεί έξυπνο έλεγχο: σκανάρει αυτόματα όλες τις εξόδους του δωματίου, βρίσκει την αντίστοιχη κλειδωμένη πόρτα και την ξεκλειδώνει χωρίς να απαιτείται καθορισμός της κατεύθυνσης από τον χρήστη.
...
ΠΧ-06 (Bonus - NPC Interaction): Ο παίκτης συναντά έναν φρουρό και γράφει give catalog to curator. Το NpcStateMachine του χαρακτήρα μεταβαίνει στην κατάσταση "happy", ενεργοποιώντας νέες γραμμές διαλόγου και ξεκλειδώνοντας τις κλειδωμένες διόδους του χώρου.
