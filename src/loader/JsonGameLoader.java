    ...
    public Room loadGame(String filePath) {
        ...
        try (Reader reader = new FileReader(filePath)) {
            ...
            for (GameDataDTO.RoomDTO rDTO : gameData.rooms) {
                ...
                // Populate autonomous NPCs and their internal state machines
                if (rDTO.npcs != null) {
                    for (GameDataDTO.NpcDTO nDTO : rDTO.npcs) {
                        NpcImpl npc = new NpcImpl(nDTO.id, nDTO.name, nDTO.description, nDTO.startingState, nDTO.requiredItem);
                        
                        // Load NPC dialogs mapped to their respective states
                        if (nDTO.dialogs != null) {
                            for (Map.Entry<String, String> entry : nDTO.dialogs.entrySet()) {
                                npc.addDialog(entry.getKey(), entry.getValue());
                            }
                        }
                        room.addNpc(npc);
                    }
